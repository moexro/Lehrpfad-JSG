...
function lp_startTracking(map, markerRefs, basehref) {
  ...
  // iOS 13+ benötigt Erlaubnis für DeviceOrientation
  if (
    typeof DeviceOrientationEvent !== "undefined" &&
    typeof DeviceOrientationEvent.requestPermission === "function"
  ) {
    // Button zum Anfordern der Orientierungserlaubnis
    const compassBtn = L.control({ position: "bottomright" });
    compassBtn.onAdd = () => {
      const btn = L.DomUtil.create("button", "lp-locate-btn lp-compass-btn");
      btn.innerHTML = `🧭`;
      btn.title = "Kompass aktivieren";
      btn.addEventListener("click", () => {
